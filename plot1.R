...
totalEmissions <- function(NIE,SCC){
	yearlyEmissions <- with(NIE, tapply(Emissions, year, sum, na.rm = T))
	...
	ytics[2] <- 0
      yticsNonZero <- ytics >0
	ytics <- ytics[yticsNonZero]

   	#Set Plot Device to plot1.p as 240x200...
    	png("plot1.png", width=240, height=200,units="mm",res=300)
    	curPlotDevice <- dev.cur()
      
	p("Ploting for PM2.5 decreased in the United States (1999 to 2008) on device(plot1.png):",toString(curPlotDevice),"...")
	...
	title("PM2.5 decreased in the United States (1999 to 2008)")
